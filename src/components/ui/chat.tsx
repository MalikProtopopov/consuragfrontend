...
import { Send, ThumbsUp, ThumbsDown, Copy, Check, Bot, User } from "lucide-react"
...
const ChatMessageBubble = React.forwardRef<HTMLDivElement, ChatMessageProps>(
  (
    { className, message, avatarUrl, avatarName, onFeedback, onCopyMessage, ...props },
    ref
  ) => {
    ...
    return (
      ...
      >
        <Avatar className="size-8 shrink-0">
          {avatarUrl ? (
            <AvatarImage src={avatarUrl} alt={avatarName || message.role} />
          ) : null}
          <AvatarFallback
            className={cn(
              isAssistant
...
const ChatInput = React.forwardRef<HTMLTextAreaElement, ChatInputProps>(
  ({ className, onSend, loading = false, ...props }, ref) => {
    ...
    return (
      <div
        className={cn(
          "flex items-end gap-2 border rounded-xl bg-surface p-2",
          className
        )}
      >
        <Textarea
            ...
            {typing && (
              <div className="flex gap-3 animate-fade-up">
                <Avatar className="size-8">
                  {avatarUrl && (
                    <AvatarImage src={avatarUrl} alt={avatarName || "Assistant"} />
                  )}
